...
def backup_table(dataset, tablename, todir, schemaonly):
    ...
    write_json_string(schema,
                      os.path.join(todir, dataset, tablename, year, month, day, 'schema.json')
                      )
    ...
    if not schemaonly:
        ...
        write_json_string(tbldef,
                          os.path.join(todir, dataset, tablename, year, month, day,'tbldef.json')
                          )
        ...
        #    output_data_name
        #])
        command="SELECT * from smartcoin-qa.dev_ops.users_cmd_history where timestamp < {} and timestamp > {}".format()
        #os.system('bq --location=asia-south1 --format=json query --use_legacy_sql=false "SELECT * from smartcoin-qa.dev_ops.users_cmd_history where timestamp < '2022-11-16-12-55-01' and timestamp > '2022-11-15-12-55-01'  "  > table_dump.json')
        res=exec_shell_command(['bq','--location=asia-south1','--format=json','query','--use_legacy_sql=false',command])
        #output=os.path.join(todir, dataset, tablename, 'data_*.json')
        _= res
        write_json_string(res,
                          os.path.join(todir, dataset, tablename, year, month, day, 'sd_logging_data.zip')
                          )
        #print(output)
        print(res)
